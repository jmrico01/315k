...
internal void* LinuxAudioRunner(void* data)
{
    ...
    while (linuxAudio->isPlaying) {
        ...
        if (ret == -EPIPE) {
            ...
            LOG_ERROR("snd_pcm_writei error\n");
        }
    }
    ...
    uint32 channels, uint32 sampleRate, uint32 periodSize, uint32 periods)
{
    ...
    if (err < 0) {
        LOG_ERROR("Error opening PCM device %s\n", pcmName);
        return false;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Failed to initialize hw params\n");
        return false;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Error setting audio access\n");
        return false;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Error setting audio format\n");
        return false;
    }
    ...
    err = snd_pcm_hw_params_set_rate_near(audio->pcmHandle, hwParams,
        &rate, 0);
    if (err < 0) {
        LOG_ERROR("Error setting sample rate\n");
        return false;
    }
    if (rate != sampleRate) {
        LOG_WARN("Sample rate %d Hz not supported by your hardware\n"
            "    ==> Using %d Hz instead.\n", sampleRate, rate);
    }
    ...
    err = snd_pcm_hw_params_set_channels(audio->pcmHandle, hwParams,
        channels);
    if (err < 0) {
        LOG_ERROR("Error setting number of channels\n");
        return false;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Error setting number of periods\n");
        return false;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Error setting HW params\n");
        return false;
    }
